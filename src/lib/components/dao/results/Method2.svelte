...
let componentKey = 0;
const color = 'warning';
const totalVotePower = FormatUtils.fmtNumber(Math.floor(ChainUtils.fromMicroAmount(method2.metadata.totalAmounts.yes + method2.metadata.totalAmounts.no)));
const totalAccounts = method2.metadata.totalVotes.yes + method2.metadata.totalVotes.no;
const scrollTop = () => {
  const getMeTo = document.getElementById("page-header");
  if (getMeTo) getMeTo.scrollIntoView({behavior: 'smooth'});
}

const inFavour = Number(((method2.metadata.totalAmounts.yes / (method2.metadata.totalAmounts.yes + method2.metadata.totalAmounts.no)) * 100).toFixed(2));
const vAg = FormatUtils.fmtNumber(Math.floor(ChainUtils.fromMicroAmount(method2.metadata.totalAmounts.no)));
const vFr = FormatUtils.fmtNumber(Math.floor(ChainUtils.fromMicroAmount(method2.metadata.totalAmounts.yes)));
...
const votes: any[] = []
onMount(async () => {
  for (const [key, value] of Object.entries(method2.value.validVotes)) {
    if (value.stackingData) {
      votes.push({
        address: key,
        locked: Number(value.stackingData.locked),
        block_height: value.txs[0].block_height,
        for: value.txs[0].token_transfer.recipient_address === 'SP00000000000003SCNSJTCHE66N2PXHX'
      })
    }
  }
})
  ...
  <div class="row">
    <div class="cols-12 text-end my-3">
      <p class="w-100 text-right">
        <a href="/" class={'text-white'} on:click|preventDefault={() => { showVotes = !showVotes }}>{#if showVotes}<Icon src={CaretDownOutline} />{:else}<Icon src={CaretUpOutline} />{/if} transactions</a>
