<script lang="ts">
	...
	import {
		...
		findStackerEventsByEventAndAddress,
	...
	const getAmountNested = () => {
		try {
			return item.amountNested
				? fmtNumber(Math.floor(Number(fmtMicroToStx(item.amountNested))))
				: 0; //ChainUtils.fromOnChainAmount(item.amount)
		} catch (err: any) {
			return 0;
		}
	};
	...
	const checkBlockHeight = () => {
		return (
			item.burnBlockHeight >= NAKAMOTO_VOTE_START_HEIGHT &&
			item.burnBlockHeight < NAKAMOTO_VOTE_STOPS_HEIGHT
		);
	};
	...
	const fetchPoxData = async () => {
		...
		entries = await findPoxEntriesByAddressAndCycle(
			item.voterProxy ? item.voterProxy : item.voter,
			78
		);
		entries = entries.concat(
			await findPoxEntriesByAddressAndCycle(item.voterProxy ? item.voterProxy : item.voter, 79)
		);
		...
		if (poxAddr && poxAddr.hashBytes) {
			...
			if (item.poxStacker) {
				...
				events = events.concat(
					await findStackerEventsByEventAndAddress('stack-stx', item.poxStacker)
				);
			}
		}
	...
	<div class="grid grid-cols-6 w-full justify-evenly text-sm mb-[-5px]">
		<div
			class={item.voter === account.stxAddress
				? 'col-span-2 text-success w-1/2 break-words'
		...
		<div class="w-full justify-end text-[#131416]/[0.64]">
			{#if coordinator}<a href="/" on:click|preventDefault={() => fetchPoxData()} target="_blank"
					>more</a
				>{/if}
		</div>
	</div>
	{#if isCoordinator(account.stxAddress)}
		<div class="grid grid-cols-6 w-full justify-evenly text-sm mb-4">
			<div class="col-span-6 text-[#131416]/[0.44]">
				{#if item.voterProxy}<span title="nested voting address"
						>Nested address: {item.voterProxy}</span
					>{/if}
				{#if item.poxStacker}<span title="nested stacker address">Stacker: {item.poxStacker}</span
					>{/if}
				{#if item.stackerData}<span title="nested stacker address">Data: {item.stackerData}</span
					>{/if}
			</div>
		</div>
	{/if}
			...
			{#each entries as entry}
				<div class="text-xs grid grid-cols-6 w-full justify-evenly">
					<div class="break-words col-span-2">
						<a href={'/dashboard/address-lookup/' + entry.stacker} target="_blank"
							>{entry.stacker}</a
						>
					</div>
					<div class="">
