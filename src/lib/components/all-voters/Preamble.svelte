<script lang="ts">
	...
	export let submissionContract:string;
	...
	<div>
		<dl class="divide-y divide-white/10">
			...
			<div class="px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
				...
				<dd class="mt-1 text-sm leading-6 text-sand-300 sm:col-span-2 sm:mt-0"><a href={explorerAddressUrl(contractId)} target="_blank">{proposalMeta.title}</a></dd>
			</div>
			<div class="px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
			...
			<div class="px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
				...
				<dd class="mt-1 text-sm leading-6 text-sand-300 sm:col-span-2 sm:mt-0"><a href={explorerAddressUrl(votingContract)} target="_blank">{votingContract.split('.')[1]}</a></dd>
			</div>
			{/if}
			<div class="px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
				<dt class="text-sm leading-6 text-white">Funding contract</dt>
				<dd class="mt-1 text-sm leading-6 text-sand-300 sm:col-span-2 sm:mt-0"><a href={explorerAddressUrl(submissionContract)} target="_blank">{submissionContract.split('.')[1]}</a></dd>
			</div>
			<div class="px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
				<dt class="text-sm leading-6 text-white">Proposal contract</dt>
				<dd class="mt-1 text-sm leading-6 text-sand-300 sm:col-span-2 sm:mt-0"><a href={explorerAddressUrl(contractId)} target="_blank">{contractId.split('.')[1]}</a></dd>
			</div>
		</dl>
	</div>
</div>
