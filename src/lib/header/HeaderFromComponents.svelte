<script lang="ts">
	...
	import { fmtMicroToStx, fmtNumber, fmtNumberStacksFloor, fmtSatoshiToBitcoin, getAddressId, getRouterInfo } from '$lib/utils';
	...
	$: balances = {
		...
		cardinalBalance: fmtSatoshiToBitcoin($sessionStore.keySets[getConfig().VITE_NETWORK]?.walletBalances?.cardinal.amount!) || '0',
		ordinalBalance: fmtSatoshiToBitcoin($sessionStore.keySets[getConfig().VITE_NETWORK]?.walletBalances?.ordinal.amount!) || '0',
		stacksBalance: fmtMicroToStx($sessionStore.keySets[getConfig().VITE_NETWORK]?.walletBalances?.stacks.amount!) || '0'
	}
