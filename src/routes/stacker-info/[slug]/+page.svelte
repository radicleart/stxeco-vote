<script lang="ts">
	...
	import { getPoxInfo, getStackerInfoLatest } from '$lib/pox_api';
	...
	onMount(async () => {
		...
		info = await getStackerInfoLatest(address)
...
<div class="flex flex-col gap-y-4 mx-auto max-w-7xl md:px-6">
	...
	<div class="flex flex-col py-10  px-10 w-full my-8 rounded-2xl bg-[#F4F3F0] ">
		...
		<div class="my-5 flex flex-col gap-y-3">
			...
			{#if showPoxEntries}
			<div>
				<p class="">How this address maps to reward slot entries in the cycle {$sbtcConfig.poxInfo.current_cycle.id}</p>
				<div class="mt-0 pt-0">
					...
					<div class="text-xs flex  w-full justify-between ">
						...
						<div class="">TotalUstx: {entry.totalUstx}</div>
					</div>
					{/each}
				</div>
			</div>
