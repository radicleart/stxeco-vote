		...
		{#if voteConcluded() || isVoting(proposal)}
			...
			<div>
				<Tabs style="underline" contentClass="py-4">
					{#if soloVoting}
						<TabItem
							class="bg-lightgray relative top-[20px] text-black rounded-t-lg border-t border-r border-l border-b-none border-x-sand-100 border-y-sand-100"
							open={method === 1}
							on:keyup={(e) => changeMethod(e, 1)}
							title="Solo Stackers"
						>
