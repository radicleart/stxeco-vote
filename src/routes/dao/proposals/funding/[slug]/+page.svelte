...
<div class="py-6 mx-auto max-w-4xl md:px-6 ">
	<div class="flex flex-col w-full my-8 max-w-4xl">
		{#if proposalEvent && proposalEvent.stage === ProposalStage.PROPOSED}
		<a href={'/dao/proposals/voting/' + proposalEvent.contractId + '?method=3'} >Vote on this proposal</a>
		{:else}
		<div class="flex flex-col gap-y-4 w-full border-[0.5px] border-gray-700 rounded-lg p-6 sm:p-10 overflow-hidden bg-gray-1000">
			...
			{:else}
			<div class="cols-12">
				<h1 class={'text-warning-500 text-2xl'}><span class={'strokeme-info'}>Proposal funding</span></h1></div>
